...
To stop containers
```
docker-compose -f docker-compose.sandbox.yaml stop
```
...
```
The `hermez-node` repository provides a mock proof server that generates mock proofs every 15 seconds. The mock prover is launched at port 3000, and it exports two endpoints:
- GET /api/status
- POST /api/input

7. Launch `hermez-node`
```
./node run --mode coord --cfg cfg.buidler.toml
```
Once the Hermez node is launched, the API can be queried at `localhost:8086`.
...
curl -X POST -d @input-344-32-256-64.json://localhost:9080/input
```
...
At this point you can stop the mock server if it is still running, and re-launch the coordinator as we saw in the previous section.
